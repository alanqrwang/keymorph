        ...
        for aug in list_of_eval_augs:
            ...
            for i in range(len(loaders[mod1])):
                if args.early_stop_eval_subjects and i == args.early_stop_eval_subjects:
                    break
                for j in range(len(loaders[mod2])):
                    if (
                    ...
                    if all([os.path.exists(p) for p in all_metrics_paths.values()]):
                        ...
                        fixed = loaders[mod1].dataset[i]
                        moving = loaders[mod2].dataset[j]
                        img_f, img_m = (
                            fixed["img"][tio.DATA][None],
                            moving["img"][tio.DATA][None],
                        )  # Hacky fix
                        if args.seg_available:
                            seg_f, seg_m = (
                                fixed["seg"][tio.DATA][None],
                        ...
                        for align_type_str, res_dict in registration_results.items():
                            ...
                            if args.seg_available:
                                ...
                                if "seg_a" in res_dict:
                                    ...
                                    raise ValueError(
                                        "No way to get aligned segmentation"
                                    )
                            ...
                            if args.save_eval_to_disk and not args.debug_mode:
                                ...
                                # Save metrics
                                metrics_path = (
                                    save_dir / f"metrics-{aug}-{align_type_str}.json"
                                )
