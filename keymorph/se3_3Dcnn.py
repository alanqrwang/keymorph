...
class SE3CNN(nn.Module):
    def __init__(
        self, pool: str = "snub_cube", res_features: str = "2_96", init: str = "delta"
    ):
            ...
            if isinstance(m, R3Conv):
                if self._init == "he":
                    init.generalized_he_init(
                        m.weights.data, m.basisexpansion, cache=True
                    )
    ...
    def build_representation(self, K: int):
        ...
        for k in range(2, K + 1):
            polinomials.append(polinomials[-1].tensor(SO3.irrep(1)))
...
if __name__ == "__main__":
    ...
    print(
        ...
        + ("YES" if torch.allclose(y, y_x90, atol=1e-5, rtol=1e-4) else "NO")
    )
    print(
        "90 degrees ROTATIONS around Y axis:  "
        + ("YES" if torch.allclose(y, y_y90, atol=1e-5, rtol=1e-4) else "NO")
    )
    print(
        "90 degrees ROTATIONS around Z axis:  "
        + ("YES" if torch.allclose(y, y_z90, atol=1e-5, rtol=1e-4) else "NO")
    )
    print(
        "180 degrees ROTATIONS around Y axis: "
        + ("YES" if torch.allclose(y, y_y180, atol=1e-5, rtol=1e-4) else "NO")
    )
    print(
        "REFLECTIONS on the Y axis:           "
        + ("YES" if torch.allclose(y, y_fx, atol=1e-5, rtol=1e-4) else "NO")
    )
    print(
        "REFLECTIONS on the Z axis:           "
        + ("YES" if torch.allclose(y, y_fy, atol=1e-5, rtol=1e-4) else "NO")
    )
